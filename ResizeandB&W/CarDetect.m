...
%     origIResize = imresize(origI, [m n]);
    ...
    toc
    tic
    [bboxesOrig, scoresOrig] = detect(detector, origI);
    toc
    ...
    origIResize = insertObjectAnnotation(origI, 'rectangle', bboxesOrig, cellstr(newScoreOrig), 'FontSize', 20);
    end

    [m,n] = size(waveletI);
    n = n/3;
    origIResize = imresize(origIResize, [m n]);
